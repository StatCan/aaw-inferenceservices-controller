...
import (
	...
	servingv1alpha2informers "github.com/kubeflow/kfserving/pkg/client/informers/exteranlversions/serving/v1alpha2"
...
type inferenceServiceSyncCallback func(*servingv1alpha2.InferenceService) error

// Controller for handling Kubeflow Profiles
type Controller struct {
	inferenceServiceLister  servingv1alpha2listers.InferenceServiceLister
	inferenceServiceSynched cache.InformerSynced

	// Sync callback will run for each object
	sync inferenceServiceSyncCallback
...
func NewController(
	inferenceServicesInformer servingv1alpha2informers.InferenceServiceInformer,
	sync inferenceServiceSyncCallback,
) *Controller {
	controller := &Controller{
		inferenceServiceLister:  inferenceServicesInformer.Lister(),
		inferenceServiceSynched: inferenceServicesInformer.Informer().HasSynced,
		sync:                    sync,
		workqueue:               workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "InferenceServices"),
	}
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	if ok := cache.WaitForCacheSync(stopCh, c.inferenceServiceSynched); !ok {
...
func (c *Controller) syncHandler(key string) error {
	...
	profile, err := c.inferenceServiceLister.Get(key)
	if err != nil {
		// The Profile resource may no longer exist, in which case we stop
		// processing.
		if errors.IsNotFound(err) {
			utilruntime.HandleError(fmt.Errorf("inference service '%s' in work queue no longer exists", key))
			return nil
		}
...
func (c *Controller) HandleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		// with it.
		if ownerRef.Kind != "InferenceService" {
			return
		}

		profile, err := c.inferenceServiceLister.Get(ownerRef.Name)
		if err != nil {
			klog.V(4).Infof("ignoring orphaned object '%s' of inference service '%s'", object.GetSelfLink(), ownerRef.Name)
			return
		}
